  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden w-full">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <div>
          <p className="text-sm font-medium text-gray-800">Create Short Link</p>
          <p className="text-xs text-gray-400">Select an anime and generate your short link</p>
        </div>
      </div>
      ...
      <div className="px-5 py-5 space-y-4">
          ...
          {!selectedAnime ? (
            <div className="relative">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 bg-gray-50 pl-9 pr-10 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition"
                />
                {fetchingAnime && (
              ...
              {showDropdown && !animeFetchError && (
                <div
                  className="absolute z-10 mt-1 w-full max-h-52 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-md"
                  onScroll={handleDropdownScroll}
                >
                  {filteredAnime.length === 0 ? (
                    <p className="py-6 text-center text-sm text-gray-400">No anime found</p>
                  ) : (
                    filteredAnime.map(anime => (
                      <button
                        key={anime._id}
                        onMouseDown={() => handleSelect(anime)}
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 border-b border-gray-100 last:border-0 transition-colors"
                      >
                        {anime.title}
                      </button>
                    ))
              ...
              {animeFetchError && (
                <p className="mt-2 rounded-lg bg-red-50 border border-red-100 px-3 py-2 text-xs text-red-500">
                  {animeFetchError}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center justify-between rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-indigo-100">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-medium text-indigo-800 truncate">{selectedAnime.title}</p>
                </div>
              </div>
              <button
                onClick={handleClearSelection}
                className="shrink-0 ml-3 rounded-md p-1.5 text-indigo-400 hover:bg-indigo-100 hover:text-indigo-600 transition-colors"
                title="Change anime"
              >
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <button
            ...
            className="shrink-0 rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-40 transition-colors"
          >
