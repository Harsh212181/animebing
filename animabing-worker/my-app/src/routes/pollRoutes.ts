...
pollRoutes.get('/active', async (c) => {
  try {
    ...
    }, { sort: { createdAt: -1 } }) as IPoll | null

    if (!poll) return c.json({ success: true, poll: null, message: 'No active poll found' })
    ...
    const pollObj: any = { ...poll, _id: poll._id?.toString(), userHasVoted: hasVoted, userVoteOption }
    ...
    return c.json({ success: true, poll: pollObj })
  } catch (err: any) {
...
pollRoutes.post('/admin/create', async (c) => {
  try {
    const { question, options, expiresAt } = await c.req.json()
    ...
    const db = await getDb(c.env.MONGODB_URI, c.env.MONGODB_DB)
    await db.collection('polls').updateMany({}, { $set: { isActive: false } })
...
pollRoutes.put('/admin/:id', async (c) => {
  try {
    const id = c.req.param('id')
    if (!isValidObjectId(id)) return c.json({ success: false, message: 'Invalid ID' }, 400)
    const { question, options, expiresAt, isActive } = await c.req.json()
    ...
    if (isActive !== undefined) {
      ...
      if (isActive) await db.collection('polls').updateMany({ _id: { $ne: toObjectId(id) } }, { $set: { isActive: false } })
    }
...
pollRoutes.put('/admin/:id/toggle', async (c) => {
  try {
    ...
    if (!poll.isActive) {
      await db.collection('polls').updateMany({ _id: { $ne: toObjectId(id) } }, { $set: { isActive: false } })
    }
